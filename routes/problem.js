...
router.get('/problem/:num',(req,res) => {
    pnum = req.params.num;
    con.query('select CONTENT from Problems where id = ?',num,(err,result) => {
        if(err) throw err;
...
router.post('/problem/:num',(req,res) => {
    ...
    async function solveCheck(pid, user){
        try{
            const results = await con.query('select * from Solved where PID = ? and USER = ?',[pid,user]);
        }
        catch(err){
            console.error(err);
        }
    ...
    con.query('select ANSWER,SCORE from Problems where PID = ?',(err,result) => {
        if(err) throw err;
        if(result === ans){
            if(solveCheck(pid,user)){
                con.query('update Users set SCORE = SCORE + ? where ID = ?',[result[0].SCORE,user]);
                res.send('<script type="text/javascript">alert("정답!!!٩(๑❛ワ❛๑)و");window.location.href("/problem");</script>');
            }
            else
